...
class Grok:
    ...
    def _load(self, extra_data: dict = None) -> None:
        if not extra_data or "cookies" not in extra_data:
    ...
    def c_request(self, next_action: str) -> None:
        ...
        if self.c_run == 0:
            ...
            match self.c_run:
                case 1:
                    data = dumps([{"anonUserId":self.anon_user}])
                case 2:
                    data = dumps([{"anonUserId":self.anon_user,**self.challenge_dict}])
    ...
    def start_convo(self, message: str, extra_data: dict = None) -> dict:
        is_continuing = extra_data and all(k in extra_data for k in ["conversationId", "parentResponseId", "cookies"])
        ...
        system_prompt = extra_data.get("system_prompt", "") if extra_data else ""
        ...
        if not is_continuing:
            conversation_data: dict = {
                ...
                'isAsyncChat': False,
                'customPersonality': system_prompt,
            }
        ...
        if "modelResponse" in convo_request.text:
            ...
            for response_dict in convo_request.text.strip().split('\n'):
                if not response_dict.strip(): continue
                try:
                    data = loads(response_dict)
                    res_obj = data.get('result', {})
                    
                    # New convo vs continue convo structure differs slightly
                    if not is_continuing:
                        token = res_obj.get('response', {}).get('token')
                        msg_node = res_obj.get('response', {}).get('modelResponse', {})
                        convo_node = res_obj.get('conversation', {})
                    else:
                        token = res_obj.get('token')
                        msg_node = res_obj.get('modelResponse', {})
                        convo_node = {} # Continue doesn't always send convoId again

                    if token: stream_response.append(token)
                    if not response and msg_node.get('message'):
                        response = msg_node['message']
                    if not conversation_id and convo_node.get('conversationId'):
                        conversation_id = convo_node['conversationId']
                    if not parent_response and msg_node.get('responseId'):
                        parent_response = msg_node['responseId']
                    if not image_urls and msg_node.get('generatedImageUrls'):
                        image_urls = msg_node['generatedImageUrls']
            ...
            return {
                ...
                "extra_data": {
                    ...
                    "conversationId": conversation_id or (extra_data["conversationId"] if is_continuing else None),
